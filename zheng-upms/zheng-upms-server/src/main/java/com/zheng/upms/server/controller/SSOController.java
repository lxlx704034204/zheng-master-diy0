...
public class SSOController extends BaseController {
    ...
    public String login(HttpServletRequest request) {
        ...
        if (StringUtils.isNotBlank(code)) {
            String backurl = request.getParameter("backurl"); // 回跳
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        ...
        Subject subject = SecurityUtils.getSubject(); // 获取当前登录user的Subject单例对象
        Session session = subject.getSession();
        ...
        if (StringUtils.isBlank(hasCode)) {
            ...
            UsernamePasswordToken usernamePasswordToken = new UsernamePasswordToken(username, password);// 使用shiro认证
            try {
                ...
                subject.login(usernamePasswordToken);// 获取当前登录user的Subject单例对象 利用令牌 登陆
